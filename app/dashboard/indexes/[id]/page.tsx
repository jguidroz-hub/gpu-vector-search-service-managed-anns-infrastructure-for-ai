...
export default function Indexes/[id]Page() {
  const [items, setItems] = useState<Indexes/[id]Item[]>([]);
  const [loading, setLoading] = useState(true);
  const [showCreate, setShowCreate] = useState(false);
  const [newTitle, setNewTitle] = useState('');

  useEffect(() => {
    fetch('/api/indexes/[id]')
      .then(r => r.json())
  ...
  const handleDelete = async (id: string) => {
    await fetch(`/api/indexes/[id]/${id}`, { method: 'DELETE' });
  ...
  return (
    <div className="max-w-4xl mx-auto p-6">
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold">Index Details</h1>
        <button
      ...
      <p className="text-gray-600 mb-6">View and configure specific index</p>
